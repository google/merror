...
TEST(MakeStatus, DescriptionFromStatus) {
  {
    ...
    EXPECT_THAT(F(),
                StatusIs(absl::StatusCode::kInternal, Eq(" \n hello \n ")));
  }
  {
    auto F = []() -> absl::Status {
    ...
    EXPECT_THAT(F(), StatusIs(absl::StatusCode::kInternal, Eq("c1 \n"
                                                              " c2\n"
                                                              "p1 \n"
                                                              " p2")));
  }
...
TEST(MakeStatus, DescriptionFromNothing) {
  static auto UNKNOWN = absl::StatusCode::kUnknown;
  {
    ...
    EXPECT_THAT(F(),
                StatusIs(UNKNOWN, MatchesRegex(std::string(
                                      R"(.*status_test\.cc:.*: MERROR\(\))"))));
  }
  {
    auto F = []() -> absl::Status {
    ...
    EXPECT_THAT(
        F(),
        StatusIs(UNKNOWN,
                 MatchesRegex(std::string(R"(.*status_test\.cc:.+: p1 .)") +
                              R"( p2.)"
                              R"(b1 .)"
                              R"( b2)")));
  }
  ...
  }
  {
    ...
    EXPECT_THAT(
        ...
        StatusIs(UNKNOWN,
                 MatchesRegex(
                     std::string(
                         R"(.*status_test\.cc:.+: MVERIFY\(2 \+ 2 == 5\).)") +
